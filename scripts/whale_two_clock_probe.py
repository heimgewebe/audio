#!/usr/bin/env python3
"""Offline A/B probe for decoupling humpback source texture from played pitch.

The legacy Morph voice replays one source-derived cycle at the MIDI pitch clock.
That shifts both temporal repetition and the complete harmonic texture whenever
an anchor's measured source clock differs from its assigned keyboard note.

This probe keeps two independent clocks:
- a clean carrier remains exactly MIDI-bound;
- the rich source-derived cycle runs at its measured source frequency.

The experiment is intentionally offline-only. Generated WAVs are evidence
artifacts and are never a production backend or default-mode mutation.
"""
...
class TwoClockProbe:
    """Render legacy and separated-clock signals from one validated morph bank."""
    ...
    def __init__(self, config: WhaleVoiceConfig | None = None) -> None:
        ...
        if tuple(item.note for item in clocks) != tuple(
            anchor.note for anchor in self.bank.anchors
        ):
    ...
    def source_clock_hz(self, timbre_note: float) -> float:
        """Log-interpolate source timing independently of the MIDI pitch clock."""
    ...
    def capped_sample(self, phase: float, timbre_note: float, harmonic_cap: int) -> float:
        ...
        if note <= self.bank.anchors[0].note:
            return self._anchor_capped_sample(
                self.bank.anchors[0], phase, harmonic_cap
            )
        if note >= self.bank.anchors[-1].note:
            return self._anchor_capped_sample(
                self.bank.anchors[-1], phase, harmonic_cap
            )
    ...
    def describe_note(self, note: int) -> dict[str, float | int]:
        ...
        return {
            ...
            "source_texture_mix": SOURCE_TEXTURE_MIX,
        }
...
def difference_energy_ratio(samples: list[float]) -> float:
    """Dependency-free high-frequency proxy: derivative energy / signal energy."""
        ...
        records.append(
            {
                **description,
                ...
                "legacy_difference_energy_ratio": legacy_edge,
                "two_clock_difference_energy_ratio": candidate_edge,
                "difference_energy_candidate_over_legacy": (
                    candidate_edge / legacy_edge if legacy_edge > 0.0 else None
                ),
            }
        )
    report = {
        ...
        "hypothesis": (
            "rich source texture should keep its measured source clock while a "
            "separate clean carrier preserves the exact MIDI pitch"
        ),
